...
# SAとRMSEの計算用の関数を加えたクラス
class MySVM(BaseDSMO):  
...
def main():
    ...
    if rank == 0:
        ...
        if plt == True:
            mysvm.plt_Data_and_Boundary_L("DSMO_plt_L")
    ...
    for i in range(size):
        if rank == i:
            ...
            print(f'[MIN, MAX]: [{min_f:.6f}, {max_f:.6f}]\n', flush=True)
    ...
    if rank == 0:
        ...
        mysvm.plt_Objective_Values("DSMO_plt_Objective_Values")
        mysvm.plt_Num_Samples("DSMO_plt_Num_Samples")
    comm.Barrier()
    
    if plt == True:
        mysvm.plt_Data_and_Boundary_D("DSMO_plt_D")
    comm.Barrier()
    ...
    # データのプロット    
    if plt == True:
        mysvm.plt_Data_and_Boundary_D("DSMO_plt_D_each")
    comm.Barrier()
